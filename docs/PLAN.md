...
| 8 | 주문 및 재고 모니터링 | 완료 |
| 9 | JSON 영속성과 재실행 복구 | 완료 |
| 10 | 전체 Acceptance Scenario | 완료 |
...
#### Cycle 8: 주문 및 재고 모니터링 — Result (GREEN)

- **상태**: Completed
- **실제 변경 파일**: `src/sample_order_system/monitor/monitor_service.py`(신규),
  `src/sample_order_system/service/monitoring_service.py`(신규 — `SampleInventoryRow` 포함),
  `src/sample_order_system/controller/monitoring_controller.py`(신규),
  `src/sample_order_system/view/monitoring_view.py`(신규),
  `src/sample_order_system/controller/main_controller.py`(메뉴 추가),
  `src/sample_order_system/view/main_menu_view.py`(메뉴 추가),
  `tests/test_monitor_service.py`(신규), `tests/test_monitoring_service.py`(신규)
- **실제 테스트 수와 결과**: 신규 8개(순수함수 6 + 서비스 2) — RED 단계에서 두 파일 모두 모듈
  부재(`ModuleNotFoundError`/`ImportError`)로 예상대로 실패 확인 → GREEN 단계에서 기존 64개
  포함 72개 전부 PASSED
- **Harness 결과**: `python scripts/verify.py` → 문법 검사 OK, `pytest -v` 72 passed, `ruff`는
  미설치로 건너뜀 → **Harness PASSED**
- **계획 대비 변경 사항**: 없음. Plan에 명시한 파일만 생성.
- **범위 이탈 여부**: 없음.
- **수동 확인**: `python main.py`로 주문 2건 접수(50, 5) → 1건 거절 → 모니터링에서 주문량
  확인(RESERVED 1건, REJECTED는 집계에서 제외) → 재고량 확인(재고 0, 유효주문 50 — 거절된 5는
  제외, 상태 `고갈`)까지 실제 실행으로 확인.
- **남은 위험/후속 작업**: 없음. Cycle 9(영속성/재실행 복구), Cycle 10(전체 Acceptance Scenario)이
  남았다.

#### Cycle 9: JSON 영속성과 재실행 복구 — Plan (RED)

- **현재 상태**: 각 기능(Cycle 1~8)마다 "저장 후 리포지토리 재생성 → 조회"를 개별적으로
  테스트해왔지만, **여러 종류의 데이터(시료+다양한 상태의 주문+생산 큐)가 함께 있는 상태로
  재시작해도 전부 정확히 복구되는지 확인하는 통합 테스트가 없다.** 또한 실제로 확인해보니
  손상된 JSON 파일을 열면 `json.JSONDecodeError`가 그대로 노출된다(트레이스백) — 앞선 4개
  PoC(DataPersistence/DataMonitor/DummyDataGenerator)는 전부 이 경우를 친절한 오류로 감쌌는데
  이 프로젝트는 아직 안 되어 있다.
- **목표**:
  1. 손상된 JSON 파일을 열면 트레이스백이 아니라 이해하기 쉬운 오류로 실패하게 한다.
  2. 시료·여러 상태의 주문·생산 큐가 뒤섞인 상태에서 리포지토리를 재생성해도(=애플리케이션
     재시작 시뮬레이션) 모든 데이터가 정확히 복구되는 것을 하나의 통합 테스트로 고정한다.
- **관련 요구사항**: PRD.md 7장(데이터 영속성 — 재시작해도 유지, 검증 실패 시 기존 파일 불변)
- **포함 범위**
  - `repository/order_system_repository.py` 확장: `OrderSystemDataError` 예외 클래스 추가.
    `_load()`에서 `json.loads` 호출을 `try/except json.JSONDecodeError`로 감싸 파일 경로와
    원인을 포함한 `OrderSystemDataError`로 다시 던진다.
  - `tests/test_repository_persistence.py`(신규): 파일 없음 시작, 손상된 JSON 처리, 그리고
    가장 중요한 **전체 라이프사이클 재시작 통합 테스트**(시료 등록 → 주문 4건을 각각 다른
    경로로 흘려보내 `REJECTED`/`CONFIRMED`(재고충분)/`RELEASE`(재고부족→생산완료→출고)/
    `PRODUCING`(재고부족, 아직 미완료) 상태로 만든 뒤 → 새 리포지토리 인스턴스로 재로드 →
    시료 재고, 각 주문 상태·수량, 생산 큐 잔여 항목까지 전부 일치하는지 확인).
- **제외 범위**: 새로운 기능(Cycle 10에서 다룰 전체 Acceptance Scenario와는 다름 — 이번은 순수
  영속성 검증이며 콘솔 UI 변경은 없다).
- **Acceptance Criteria**
  1. 데이터 파일이 없으면 빈 상태로 시작한다(회귀 확인).
  2. 손상된 JSON 파일을 열면 `OrderSystemDataError`로 실패하고 메시지에 파일 경로가 포함된다.
  3. 시료 등록 + 주문 4건(각각 REJECTED/CONFIRMED/RELEASE/PRODUCING 경로)을 만든 뒤 리포지토리를
     재생성해도 시료 재고, 모든 주문의 상태·필드, 생산 큐(미완료 1건 잔존)가 재생성 전과 완전히
     동일하다.
  4. 위 통합 시나리오에서 재생성된 리포지토리로 추가 조작(예: 잔여 `PRODUCING` 주문의 생산 완료)이
     이어서 정상 동작한다(복구된 상태가 "살아있는" 상태임을 확인).
  5. `python main.py`를 같은 데이터 파일로 두 번 실행해, 첫 실행에서 등록한 시료/주문이 두 번째
     실행에서도 그대로 조회됨을 수동으로 확인한다.
- **예정 테스트 목록** (모두 `tmp_path` 사용)
  - `tests/test_repository_persistence.py`: 파일 없음 시작, 손상된 JSON 시 `OrderSystemDataError`,
    전체 라이프사이클 재시작 통합 테스트, 복구 후 추가 조작 가능 확인
- **구현 접근 방식**: DataPersistence/DummyDataGenerator PoC의 `DataFileError`/`InvalidDataFileError`
  패턴(파일 경로와 원인을 포함한 명확한 메시지)을 참고해 `OrderSystemDataError`를 새로 작성한다.
- **변경 예정 파일**: `src/sample_order_system/repository/order_system_repository.py`(확장),
  `tests/test_repository_persistence.py`(신규)
- **Harness 명령**: `python scripts/verify.py`
- **RED/GREEN Commit 계획**: `RED 9: plan persistence and restart recovery` (PLAN.md만) → 승인 →
  `GREEN 9: implement persistence and restart recovery`
- **완료 조건**: Acceptance Criteria 5개 모두 충족, `pytest` 전체 통과, `python scripts/verify.py`
  성공, `python main.py`를 두 번 실행해 재시작 복구 확인 후 GREEN Commit.

#### Cycle 9: JSON 영속성과 재실행 복구 — Result (GREEN)

- **상태**: Completed
- **실제 변경 파일**: `src/sample_order_system/repository/order_system_repository.py`(확장 —
  `OrderSystemDataError` 추가, `_load()`의 `json.JSONDecodeError` 래핑),
  `tests/test_repository_persistence.py`(신규)
- **실제 테스트 수와 결과**: 신규 4개 — RED 단계에서 `OrderSystemDataError` 부재로
  `ImportError`로 예상대로 실패 확인 → GREEN 단계에서 기존 72개 포함 76개 전부 PASSED
- **Harness 결과**: `python scripts/verify.py` → 문법 검사 OK, `pytest -v` 76 passed, `ruff`는
  미설치로 건너뜀 → **Harness PASSED**
- **계획 대비 변경 사항**: 없음. Plan에 명시한 파일만 생성.
- **범위 이탈 여부**: 없음.
- **수동 확인**: `python main.py`를 같은 데이터 파일로 두 번 실행 — 1차에서 등록한 시료(`S-001`)와
  주문(`ORD-0001`)이 2차 실행에서 그대로 조회됨을 확인. 손상된 JSON 파일을 직접 만들어 로드
  시도 시 트레이스백 없이 `OrderSystemDataError`(파일 경로 포함)로 처리됨을 확인.
- **남은 위험/후속 작업**: 없음. Cycle 10(전체 Acceptance Scenario)만 남았다.

#### Cycle 10: 전체 Acceptance Scenario — Plan (RED)

- **현재 상태**: Cycle 1~9에서 각 기능은 개별적으로, Cycle 9에서는 "여러 주문 상태가 섞인 채로
  재시작"까지 검증했다. 하지만 **서비스 계층 전체를 하나의 연속된 시나리오(PRD 배경 스토리에
  대응하는 등록→주문→승인/거절→생산→출고→모니터링)로 실행했을 때 최종 결과가 전부 맞는지**와,
  **콘솔(`main.py`)을 실제로 구동해 모든 메뉴를 한 번씩 통과시켜도 크래시 없이 동작하는지를
  자동화된 회귀 테스트로 고정한 적은 없다**(지금까지는 매 Cycle마다 내가 수동으로 확인).
- **목표**: 이 두 가지를 자동화된 pytest로 고정해, 이후 리팩터링이나 추가 변경이 생겨도 전체
  흐름이 깨지지 않았는지 한 번에 확인할 수 있게 한다.
- **관련 요구사항**: PRD.md 전체(특히 5장 상태 흐름, 6장 기능 명세), CLAUDE.md 3장(Harness)
- **특수 사항 — 이번 Cycle은 새 프로덕션 코드가 필요 없을 가능성이 높다.** Cycle 1~9에서 만든
  기능을 조합해서 실행하는 테스트이므로, 버그가 없다면 테스트가 "처음부터" 통과할 수 있다.
  `SKILL.md`의 "GREEN 없이 이미 통과하는 테스트" 절차를 따른다 — 테스트가 바로 통과하면, 어떤
  기존 기능이 그 동작을 커버하는지 설명하고, 우연이 아니라 유효한 회귀 테스트인지 확인받은 뒤
  GREEN 단계 없이 REVIEW로 넘어간다. 만약 실행 중 실제 버그(잘못 계산된 값, 깨지는 메뉴 흐름 등)를
  발견하면 그때는 통상적인 RED→GREEN으로 그 버그를 고친다.
- **포함 범위**
  - `tests/test_acceptance_scenario.py`(신규): 서비스 계층(`sample_service`, `order_service`,
    `production_service`, `shipment_service`, `monitoring_service`)만 사용해 아래 시나리오를
    실행하고 각 단계의 최종 상태를 검증한다.
    1. 시료 2종 등록(하나는 초기 재고 0, 하나는 초기 재고 있음).
    2. 주문 4건을 각각 다른 경로로: 거절(`REJECTED`) / 재고 충분 승인 후 즉시 출고(`RELEASE`) /
       재고 부족 승인→생산 완료→출고(`RELEASE`) / 미승인 상태로 대기(`RESERVED`).
    3. 모니터링 집계(상태별 건수, 시료별 재고와 여유/부족/고갈 상태)가 시나리오 종료 시점의
       실제 상태와 정확히 일치하는지 확인.
  - `tests/test_console_acceptance.py`(신규): `subprocess`로 `python main.py`를 실제 구동해
    시료 등록 → 주문 접수 → 승인 → 생산 완료 → 출고 → 모니터링까지 메뉴를 한 번씩 실제로
    통과시킨다(입력은 표준입력으로 스크립트 전달, 작업 디렉터리는 `tmp_path`로 격리해 실제
    `data/`를 건드리지 않는다). 정상 종료(exit code 0, 트레이스백 없음)와 저장된 JSON의 최종
    상태(`RELEASE` 등)를 확인한다.
- **제외 범위**: 새로운 기능. 이 Cycle에서 버그가 발견되지 않는 한 프로덕션 코드는 변경하지 않는다.
- **Acceptance Criteria**
  1. 서비스 계층만으로 등록→여러 경로의 주문→생산완료→출고까지 실행하면 각 주문의 최종 상태가
     시나리오 설계와 정확히 일치한다.
  2. 모니터링 집계(상태별 건수, 시료별 재고 상태)가 시나리오 종료 시점과 정확히 일치한다.
  3. `subprocess`로 구동한 콘솔 세션이 모든 메뉴를 한 번씩 통과하고도 크래시 없이 정상
     종료하며, 저장된 JSON에 최종 상태가 반영된다.
  4. 전체 `pytest` 스위트(Cycle 0~10 전부, 기존 76개 + 신규)가 회귀 없이 통과한다.
  5. `python scripts/verify.py`가 성공한다.
- **예정 테스트 목록**
  - `tests/test_acceptance_scenario.py::test_full_acceptance_scenario_across_all_features`
  - `tests/test_console_acceptance.py::test_full_console_acceptance_scenario`
- **구현 접근 방식**: 순수하게 지금까지 만든 서비스/리포지토리/콘솔 진입점을 조합해서 실행하는
  통합 테스트이며, 새 모듈이나 클래스를 만들지 않는다.
- **변경 예정 파일**: `tests/test_acceptance_scenario.py`(신규),
  `tests/test_console_acceptance.py`(신규). (버그 발견 시에만 관련 `src/` 파일 수정)
- **Harness 명령**: `python scripts/verify.py`
- **RED/GREEN Commit 계획**: `RED 10: plan full acceptance scenario` (PLAN.md만) → 승인 → 테스트
  작성·실행 → 버그 없이 바로 통과하면 `SKILL.md`의 특수 케이스에 따라 GREEN 단계 없이 바로
  REVIEW·Commit(`REVIEW 10: full acceptance scenario already covered by existing features`),
  버그가 발견되면 그 버그에 대해 통상적인 `GREEN 10: fix <bug>`로 진행.
- **완료 조건**: Acceptance Criteria 5개 모두 충족, 전체 `pytest` 통과, `python scripts/verify.py`
  성공.

#### Cycle 10: 전체 Acceptance Scenario — Result (REVIEW, GREEN 없이 커밋)

- **상태**: Completed
- **실제 변경 파일**: `tests/test_acceptance_scenario.py`(신규),
  `tests/test_console_acceptance.py`(신규). **`src/` 프로덕션 코드 변경 없음.**
- **왜 GREEN 없이 통과했는가**: 서비스 계층 시나리오(`test_full_acceptance_scenario_across_all_features`)는
  Cycle 1~9에서 각각 구현·검증한 등록/주문/승인(충분·부족)/생산완료/출고/모니터링 기능을 그대로
  조합해 실행한 것이며, 각 기능이 개별적으로 옳을 뿐 아니라 "차감은 출고 시점 단 한 곳"이라는
  Cycle 4~9에서 일관되게 지켜온 단일 규칙 덕분에 여러 기능이 합쳐졌을 때도 재고·상태 계산이
  정확히 맞물린다. 우연이 아니라 이 반복적으로 강제해온 설계 원칙의 자연스러운 결과이므로
  유효한 회귀 테스트로 판단해 승인받았다.
- **발견한 문제(비즈니스 로직 아님)**: `test_console_acceptance.py`가 처음엔 Windows 콘솔
  코드페이지(cp949)와 subprocess UTF-8 강제 디코딩 충돌로 `UnicodeDecodeError`가 났다.
  `env={"PYTHONIOENCODING": "utf-8"}`을 subprocess 호출에 추가해 해결(테스트 의도·assertion은
  변경 없음, `main.py`/`src/` 코드와 무관).
- **실제 테스트 수와 결과**: 신규 2개(서비스 계층 시나리오 1 + 콘솔 시나리오 1) — 서비스 계층은
  즉시 PASSED, 콘솔은 인코딩 수정 후 PASSED → 기존 76개 포함 전체 78개 PASSED
- **Harness 결과**: `python scripts/verify.py` → 문법 검사 OK, `pytest -v` 78 passed(회귀 없음),
  `ruff`는 미설치로 건너뜀 → **Harness PASSED**
- **계획 대비 변경 사항**: 없음(인코딩 수정은 Plan에서 예고한 "버그 발견 시 그 자리에서 처리"
  절차 안에서 이뤄짐).
- **범위 이탈 여부**: 없음.
- **남은 위험/후속 작업**: 없음. Cycle 0~10 전체 완료.

#### Cycle 11: 메인 메뉴 요약 정보 — Plan (RED)

- **현재 상태**: `main_menu_view.py`는 메뉴 목록만 출력하고, PDF p10("전체 시료에 대한 요약
  정보를 확인")과 PRD.md §6.1이 명시한 요약 정보(등록 시료 수, 총 재고, 전체 주문 수, 생산라인
  대기 건수)는 표시하지 않는다. Cycle 10 이후 셀프 검증 과정에서 발견된 갭.
- **목표**: 메인 메뉴 표시 시 위 4개 요약 값을 함께 출력한다.
- **관련 요구사항**: PRD.md §6.1, `OrderSystemProject.pdf` p10(메인 메뉴 기능 명세)
- **포함 범위**
  - 신규 순수 함수 모듈 `src/sample_order_system/service/main_menu_summary_service.py`:
    `MainMenuSummary` dataclass(`sample_count`, `total_inventory`, `order_count`,
    `production_queue_count`) + `get_main_menu_summary(repository)`
  - `MainMenuView.show_summary(summary)` 메서드 추가 — 요약 라인 출력
  - `MainController.run()`에서 매 루프마다 `show_menu()` 호출 전 요약 계산·출력
- **제외 범위**: 모니터링 메뉴([6])의 기존 상태별 집계/재고 리포트 로직 변경 없음(완전히
  별개 경로). 메인 메뉴의 요약은 참고용 스냅샷일 뿐 상세 조회는 여전히 모니터링 메뉴가 담당.
- **Acceptance Criteria**
  1. `get_main_menu_summary`가 등록된 시료 수, 전체 시료 재고 합계, 전체 주문 수, 생산큐 대기
     건수를 정확히 계산한다.
  2. `MainMenuView.show_summary`가 4개 값을 모두 콘솔에 출력한다.
  3. 콘솔 전체 구동 시 메인 메뉴에 요약 정보가 실제로 표시된다.
  4. 기존 78개 테스트가 회귀 없이 통과한다.
  5. `python scripts/verify.py`가 성공한다.
- **예정 테스트 목록**
  - `tests/test_main_menu_summary_service.py::test_get_main_menu_summary_computes_totals`
  - `tests/test_main_menu_view.py::test_show_summary_prints_all_fields`
  - `tests/test_console_acceptance.py`에 요약 라벨 출력 여부를 확인하는 assertion 추가
- **구현 접근 방식**: 기존 서비스 계층 패턴(순수 함수 + 얇은 서비스 래퍼)을 그대로 따른다.
  `MainController`는 매 루프마다 `main_menu_summary_service.get_main_menu_summary(self.repository)`를
  호출해 `MainMenuView.show_summary`에 전달한다.
- **변경 예정 파일**: `src/sample_order_system/service/main_menu_summary_service.py`(신규),
  `tests/test_main_menu_summary_service.py`(신규), `tests/test_main_menu_view.py`(신규),
  `src/sample_order_system/view/main_menu_view.py`(수정), `src/sample_order_system/controller/main_controller.py`(수정),
  `tests/test_console_acceptance.py`(수정)
- **Harness 명령**: `python scripts/verify.py`
- **RED/GREEN Commit 계획**: `RED 11: plan main menu summary`(PLAN.md만) → 승인 → 테스트 작성·RED
  확인 → `GREEN 11: implement main menu summary`
- **완료 조건**: Acceptance Criteria 5개 모두 충족, 전체 `pytest` 통과, `python scripts/verify.py`
  성공.

#### Cycle 11: 메인 메뉴 요약 정보 — Result (GREEN)

- **상태**: Completed
- **실제 변경 파일**: `src/sample_order_system/service/main_menu_summary_service.py`(신규),
  `tests/test_main_menu_summary_service.py`(신규), `tests/test_main_menu_view.py`(신규),
  `src/sample_order_system/view/main_menu_view.py`(수정 — `show_summary` 추가),
  `src/sample_order_system/controller/main_controller.py`(수정 — 루프마다 요약 계산·출력),
  `tests/test_console_acceptance.py`(수정 — 요약 라벨 assertion 추가)
- **RED 확인**: 신규 테스트 3건 모두 올바른 이유로 실패 확인 —
  `main_menu_summary_service` 모듈 부재로 인한 `ImportError` 2건(서비스/뷰 테스트),
  콘솔 출력에 요약 라벨이 없어 실패한 1건(콘솔 acceptance test 확장분).
- **구현**: 계획대로 순수 함수(`MainMenuSummary` dataclass + `get_main_menu_summary`)를
  서비스 계층에 추가하고, `MainMenuView.show_summary`가 4개 값을 한 줄로 출력하도록 구현.
  `MainController.run()`이 매 루프마다 `show_menu()` 호출 전에 요약을 계산·출력하도록 배선.
- **실제 테스트 수와 결과**: 신규 3개(서비스 계산 로직 2 + 뷰 출력 1) + 기존
  `test_console_acceptance.py` 확장분 → 전체 81개 PASSED(기존 78개 + 신규 3개, 회귀 없음).
- **Harness 결과**: `python scripts/verify.py` → 문법 검사 OK, `pytest -v` 81 passed,
  `ruff`는 미설치로 건너뜀 → **Harness PASSED**
- **수동 검증**: `main.py`를 직접 구동해 메인 메뉴 진입 시 요약 라인("등록 시료 N종  총 재고 N
  전체 주문 N건  생산라인 대기 N건")이 실제로 출력됨을 확인. Windows 콘솔에서 텍스트 도구로
  캡처할 때 cp949 코드페이지로 인해 글자가 깨져 보이는 것은 Cycle 10에서도 확인된 동일한
  표시 환경 이슈이며, `PYTHONIOENCODING=utf-8` subprocess 테스트(81개 중 포함)가 정확한 한글
  문자열 매칭으로 실제 동작을 검증했다.
- **계획 대비 변경 사항**: 없음.
- **범위 이탈 여부**: 없음(모니터링 메뉴의 기존 로직은 손대지 않음).
- **남은 위험/후속 작업**: 없음.

## 4. 전체 Cycle 완료 요약

Cycle 0~11 전부 완료되었다. Cycle 11은 셀프 검증 중 발견된 메인 메뉴 요약 정보 누락 갭을
메우기 위해 추가되었다. 최종 커밋 이력은 각 Cycle의 RED/GREEN(또는 REVIEW) 커밋으로
`git log`에서 확인 가능하며, `python scripts/verify.py` 실행 시 전체 81개 테스트가 통과한다.
